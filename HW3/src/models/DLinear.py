...
class DLinear(MLForecastModel):
    ...
    def _fit(self, X: np.ndarray) -> None:
        model_optim = torch.optim.Adam(self.model.parameters(), lr=0.0003)
        ...
        for i in range(n_epochs):
            ...
            print("Epoch: {0} Steps: {1} Train Loss: {2:.2f}".format(i, n_steps, total_loss))
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        seasonal_init, trend_init = seasonal_init.permute(0, 2, 1).to(torch.float), trend_init.permute(0, 2, 1).to(
            torch.float)
        if self.individual:
